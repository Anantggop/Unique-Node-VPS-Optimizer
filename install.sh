...
INSTALL_DIR="/opt/unique-node-vps-optimizer"
...
function show_banner() {
    ...
    echo "╔══════════════════════════════════════════════════════════════════╗"
    echo "║                                                                  ║"
    echo "║          🚀🚀  WELCOME TO UNIQUE NODE VPS OPTIMIZER 🚀🚀         ║"
    echo "║                                                                  ║"
    echo "╠══════════════════════════════════════════════════════════════════╣"
...
function install_or_update() {
    if [ -d "$INSTALL_DIR" ]; then
        echo -e "${YELLOW}Previous installation found. Removing old version...${NC}"
        rm -rf "$INSTALL_DIR"
    else
    ...
    echo -e "${GREEN}Cloning Unique Node VPS Optimizer repository...${NC}"
    git clone https://github.com/Anantggop/Unique-Node-VPS-Optimizer.git "$INSTALL_DIR"

    echo -e "${GREEN}Setting up...${NC}"
    cd "$INSTALL_DIR"
    chmod +x unique.sh

    echo -e "\n${CYAN}Installation/Update completed successfully!${NC}"
}

# Start Optimizer
function start_optimizer() {
    echo -e "${YELLOW}Launching Unique Node VPS Optimizer...${NC}"
    sleep 2
    cd "$INSTALL_DIR"
    ./unique.sh
}
...
function delete_optimizer() {
    echo -e "${RED}Deleting Unique Node VPS Optimizer...${NC}"
    loading_animation
...
echo -e "${GREEN}3) Delete Unique Node VPS Optimizer${NC}"
